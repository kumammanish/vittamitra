...
def analyze_deductions(
    ...
    is_metro: bool = True,
    interest_income_result: dict = None,
) -> dict:
    """
    ...
    interest_income_result : dict   – Output of other_income.compute_interest_income()
                                      If provided, adds 80TTA/80TTB to analysis
    ...
    # ── 80TTA / 80TTB – Interest income deductions ─────────────────────────
    if interest_income_result:
        tta = interest_income_result.get("deduction_80tta", 0)
        ttb = interest_income_result.get("deduction_80ttb", 0)
        if tta > 0:
            analysis["80TTA"] = {
                "label":       "Sec 80TTA – Savings Interest Deduction",
                "limit":       10000,
                "claimed":     tta,
                "remaining":   max(0, 10000 - tta),
                "utilized_pct": round(tta / 10000 * 100, 1),
            }
        if ttb > 0:
            analysis["80TTB"] = {
                "label":       "Sec 80TTB – Senior Citizen Interest Deduction",
                "limit":       50000,
                "claimed":     ttb,
                "remaining":   max(0, 50000 - ttb),
                "utilized_pct": round(ttb / 50000 * 100, 1),
            }
